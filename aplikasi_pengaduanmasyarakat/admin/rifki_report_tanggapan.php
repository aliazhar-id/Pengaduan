  ...
  <div id="wrapper">
      ...
      <div id="content">
        ...
        <div id="form-tahun">
            ...
            <select name="tahun" class="form-control">
                <option value="">Pilih</option>
                <?php
                $query = "SELECT YEAR(rifki_tgl_tanggapan) AS tahun FROM rifki_tanggapan GROUP BY YEAR(rifki_tgl_tanggapan)"; // Tampilkan tahun sesuai di tabel transaksi
        ...
        <a href="rifki_report_tanggapan.php" class="btn mb-1 btn-rounded btn-outline-success">Reset Filter</a>
    </form>
    <hr />
    ...
    if(isset($_GET['filter']) && ! empty($_GET['filter'])){ // Cek apakah user telah memilih filter dan klik tombol tampilkan
        ...
        if($filter == '1'){ // Jika filter nya 1 (per tanggal)
            ...
            echo '<a href="rifki_printtanggapan.php?filter=1&tanggal='.$_GET['tanggal'].'" class="btn mb-1 btn-rounded btn-outline-info">Cetak PDF</a><br /><br />';

            $query = "SELECT * FROM rifki_tanggapan WHERE DATE(rifki_tgl_tanggapan)='".$_GET['tanggal']."'"; // Tampilkan data transaksi sesuai tanggal yang diinput oleh user pada filter
        }else if($filter == '2'){ // Jika filter nya 2 (per bulan)
            $nama_bulan = array('', 'Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember');

            echo '<b>Data Tanggapan Bulan '.$nama_bulan[$_GET['bulan']].' '.$_GET['tahun'].'</b><br /><br />';
            echo '<a href="rifki_printtanggapan.php?filter=2&bulan='.$_GET['bulan'].'&tahun='.$_GET['tahun'].'" class="btn mb-1 btn-rounded btn-outline-info">Cetak PDF</a><br /><br />';

            $query = "SELECT * FROM rifki_tanggapan WHERE MONTH(rifki_tgl_tanggapan)='".$_GET['bulan']."' AND YEAR(rifki_tgl_tanggapan)='".$_GET['tahun']."'"; // Tampilkan data transaksi sesuai bulan dan tahun yang diinput oleh user pada filter
        }else { // Jika filter nya 3 (per tahun)
            echo '<b>Data Pengaduan Tahun '.$_GET['tahun'].'</b><br /><br />';
            echo '<a href="rifki_printtanggapan.php?filter=3&tahun='.$_GET['tahun'].'" class="btn mb-1 btn-rounded btn-outline-info">Cetak PDF</a><br /><br />';

            $query = "SELECT * FROM rifki_tanggapan WHERE YEAR(rifki_tgl_tanggapan)='".$_GET['tahun']."'"; // Tampilkan data transaksi sesuai tahun yang diinput oleh user pada filter
        
            }
    }else{ // Jika user tidak mengklik tombol tampilkan
        echo '<b>Semua Data Tanggapan</b><br /><br />';
        echo '<a href="rifki_printtanggapan.php" class="btn mb-1 btn-rounded btn-outline-info">Cetak PDF</a><br /><br />';

        $query = "SELECT * FROM rifki_tanggapan ORDER BY rifki_tgl_tanggapan"; // Tampilkan semua data transaksi diurutkan berdasarkan tanggal
    }
    ...
    if($row > 0){ // Jika jumlah data lebih dari 0 (Berarti jika data ada)
        while($data = mysqli_fetch_array($sql)){ // Ambil semua data dari hasil eksekusi $sql
            $tgl = date('d-m-Y', strtotime($data['rifki_tgl_tanggapan'])); // Ubah format tanggal jadi dd-mm-yyyy
            $no_urut++;

            echo "<tr>";
            echo "<td>".$no_urut."</td>";
            echo "<td>".$tgl."</td>";
            echo "<td>".$data['rifki_tanggapan']."</td>";
            echo "</tr>";
        }
