...
<body>
	...
	if(isset($_GET['filter']) && ! empty($_GET['filter'])){ // Cek apakah user telah memilih filter
		...
		if($filter == '1'){ // Jika filter nya 1 (per tanggal)
			...
			$query = "SELECT * FROM rifki_tanggapan WHERE DATE(rifki_tgl_tanggapan)='".$_GET['tanggal']."'"; // Tampilkan data pengaduan sesuai tanggal yang diinput oleh user pada filter
		}else if($filter == '2'){ // Jika filter nya 2 (per bulan)
			$nama_bulan = array('', 'Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember');

			echo '<b>Data Tanggapan Bulan '.$nama_bulan[$_GET['bulan']].' '.$_GET['tahun'].'</b><br /><br />';

			$query = "SELECT * FROM rifki_tanggapan WHERE MONTH(rifki_tgl_tanggapan)='".$_GET['bulan']."' AND YEAR(rifki_tgl_tanggapan)='".$_GET['tahun']."'"; // Tampilkan data pengaduan sesuai bulan dan tahun yang diinput oleh user pada filter
		}else{ // Jika filter nya 3 (per tahun)
			echo '<b>Data Tanggapan Tahun '.$_GET['tahun'].'</b><br /><br />';

			$query = "SELECT * FROM rifki_tanggapan WHERE YEAR(rifki_tgl_tanggapan)='".$_GET['tahun']."'"; // Tampilkan data pengaduan sesuai tahun yang diinput oleh user pada filter
		}
	}else{ // Jika user tidak memilih filter

		$query = "SELECT * FROM rifki_tanggapan ORDER BY rifki_tgl_tanggapan"; // Tampilkan semua data pengaduan diurutkan berdasarkan tanggal
	}
	?>
...
$pdf->WriteHTML($html);
$pdf->Output('Data Tanggapan Admin.pdf', 'D');
?>
