  ...
  <div id="wrapper">
      ...
      <div id="content">
        ...
        <div id="form-tahun">
            ...
            <select name="tahun" class="form-control">
                <option value="">Pilih</option>
                <?php
                $query = "SELECT YEAR(rifki_tgl) AS tahun FROM rifki_tanggapan GROUP BY YEAR(rifki_tgl)"; // Tampilkan tahun sesuai di tabel transaksi
        ...
        <a href="rifki_reporttanggapanptgs.php" class="btn mb-1 btn-rounded btn-outline-success">Reset Filter</a>
    </form>
    <hr />
    ...
    if(isset($_GET['filter']) && ! empty($_GET['filter'])){ // Cek apakah user telah memilih filter dan klik tombol tampilkan
        ...
        if($filter == '1'){ // Jika filter nya 1 (per tanggal)
            ...
            echo '<a href="rifki_printtanggapanptgs.php?filter=1&tanggal='.$_GET['tanggal'].'" class="btn mb-1 btn-rounded btn-outline-info">Cetak PDF</a><br /><br />';

            $query = "SELECT * FROM rifki_tanggapanptgs WHERE DATE(rifki_tgl)='".$_GET['tanggal']."'"; // Tampilkan data transaksi sesuai tanggal yang diinput oleh user pada filter
        }else if($filter == '2'){ // Jika filter nya 2 (per bulan)
            $nama_bulan = array('', 'Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember');

            echo '<b>Data Tanggapan Bulan '.$nama_bulan[$_GET['bulan']].' '.$_GET['tahun'].'</b><br /><br />';
            echo '<a href="rifki_printtanggapanptgs.php?filter=2&bulan='.$_GET['bulan'].'&tahun='.$_GET['tahun'].'" class="btn mb-1 btn-rounded btn-outline-info">Cetak PDF</a><br /><br />';

            $query = "SELECT * FROM rifki_tanggapanptgs WHERE MONTH(rifki_tgl)='".$_GET['bulan']."' AND YEAR(rifki_tgl)='".$_GET['tahun']."'"; // Tampilkan data transaksi sesuai bulan dan tahun yang diinput oleh user pada filter
        }else { // Jika filter nya 3 (per tahun)
            echo '<b>Data Pengaduan Tahun '.$_GET['tahun'].'</b><br /><br />';
            echo '<a href="rifki_printtanggapanptgs.php?filter=3&tahun='.$_GET['tahun'].'" class="btn mb-1 btn-rounded btn-outline-info">Cetak PDF</a><br /><br />';

            $query = "SELECT * FROM rifki_tanggapanptgs WHERE YEAR(rifki_tgl='".$_GET['tahun']."'"; // Tampilkan data transaksi sesuai tahun yang diinput oleh user pada filter
        
            }
    }else{ // Jika user tidak mengklik tombol tampilkan
        echo '<b>Semua Data Tanggapan</b><br /><br />';
        echo '<a href="rifki_printtanggapanptgs.php" class="btn mb-1 btn-rounded btn-outline-info">Cetak PDF</a><br /><br />';

        $query = "SELECT * FROM rifki_tanggapanptgs ORDER BY rifki_tgl"; // Tampilkan semua data transaksi diurutkan berdasarkan tanggal
    }
